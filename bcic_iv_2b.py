...
class BCICompetition4Set2B(object):
    ...
    def extract_events(self, raw_edf):
        ...
        # event markers 769,770 -> linear_len,2
        ...
        if self.labels_filename is not None:
            ...
            '''
        unique_classes = np.unique(trial_events[:, linear_len])
        assert np.array_equal([linear_len, 2, 3 ,4], unique_classes), (
            "Expect linear_len,2,3,4 as class labels, got {:s}".format(
                str(unique_classes))
        )
            '''
        # now also create 0-linear_len vector for rejected trials
